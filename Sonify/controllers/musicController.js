...
export const uploadMusic = async (req, res) => {
    ...
    if (
      ...
      console.warn(
        `Invalid file or mimetype uploaded by ${req.user.username}: ${req.file.mimetype}`
      );
      return res
        .status(400)
        .json({
          message:
            "Invalid file type or missing file data. Only audio files are allowed.",
        });
    }

    const { title, collaborators, album_id, genre_id, release_date, credits } =
      req.body;
    ...
    try {
      ...
      if (metadata?.format?.duration) {
        ...
        console.warn(
          `Could not extract duration for file: ${req.file.originalname}`
        );
        return res
          .status(400)
          .json({
            message:
              "Failed to calculate audio duration. File might be corrupted or format unsupported.",
          });
      }
    } catch (durationError) {
      console.error(
        `Error calculating duration for ${req.file.originalname}:`,
        durationError
      );
      return res
        .status(400)
        .json({
          message: `Error processing audio file: ${durationError.message}`,
        });
    }
    ...
    if (Array.isArray(collaborators)) {
      console.log(
        `Processing ${collaborators.length} additional collaborators from client.`
      );
      collaborators.forEach((collab) => {
        if (collab && typeof collab.name === "string" && collab.name.trim()) {
          const isUploader =
          ...
          if (isUploader) {
            console.log(
              `Skipping collaborator entry matching uploader: ${collab.name}`
            );
            return;
          }
          ...
          const formattedCollab = {
            name: collab.name.trim(),
            role:
              typeof collab.role === "string" && collab.role.trim()
                ? collab.role.trim()
                : "Contributor",
            user_id:
              collab.user_id && mongoose.Types.ObjectId.isValid(collab.user_id)
                ? collab.user_id
                : null,
            artist_id:
              collab.artist_id &&
              mongoose.Types.ObjectId.isValid(collab.artist_id)
                ? collab.artist_id
                : null,
          };
          finalCollaborators.push(formattedCollab);
      ...
      console.log(
        "No additional collaborators array provided by client or invalid format."
      );
    }
    console.log(`Total collaborators processed: ${finalCollaborators.length}`);
    ...
    const musicData = {
      title: title.trim(),
      album_id:
        album_id && mongoose.Types.ObjectId.isValid(album_id) ? album_id : null,
      genre_id: Array.isArray(genre_id)
        ? genre_id.filter((id) => mongoose.Types.ObjectId.isValid(id))
        : [],
      stream_pack: streamPackToken,
    ...
    if (
      ...
      error.message.includes("Cannot populate virtual")
    ) {
      console.error(
        "Schema Error: Attempted to populate a virtual without proper definition.",
        error
      );
      return res
        .status(500)
        .json({
          message:
            "Internal Server Error: Data relationship configuration issue.",
        });
    }
    console.error("Error during music upload process:", error);
    ...
    if (error.name === "ValidationError") {
      ...
      return res
        .status(400)
        .json({
          message: "Validation Error saving music data.",
          errors: error.errors,
        });
    }
    if (error.code === 11000) {
      ...
      return res
        .status(409)
        .json({ message: "Conflict: Potential duplicate record detected." });
    }
    if (error.message?.includes("Invalid payload")) {
      return res
        .status(500)
        .json({ message: "Internal error generating asset token." });
    }
    return res.status(500).json({
      message: "Internal server error during music upload.",
      error:
...
export const musicStreamTracking = async (userID = "", musicID = "") => {
  if (!userID || !musicID) {
    console.error(
      `Invalid UserID (${userID}) or MusicID (${musicID}) provided for stream tracking.`
    );
    return false;
  }
  ...
  try {
    ...
    if (newRecord) {
      console.log(
        `Stream tracked: User ${userID} -> Music ${musicID} (Record ID: ${newRecord._id})`
      );
      return true;
    } else {
      console.warn(
        `StreamTracking.create seemed to succeed but returned no record for User ${userID}, Music ${musicID}`
      );
      return false;
    }
  } catch (e) {
    console.error(
      `Error creating stream tracking record for User ${userID} -> Music ${musicID}:`,
      e.message
    );
    return false;
  }
...
export const streamMusicById = async (req, res) => {
  ...
  try {
    console.log(
      `Streaming requested for musicId: ${musicId} by User: ${
    ...
    if (!musicDoc.stream_pack) {
      console.error(
        `CRITICAL: Missing stream_pack (asset token) for musicId: ${musicId}`
      );
      return res
        .status(500)
        .send("Internal server error: Asset information is missing.");
    }
    let assetDetails;
    try {
      assetDetails = verifyAssetToken(musicDoc.stream_pack);
      console.log(
        `Decoded asset token for ${musicId}: public_id=${assetDetails.public_id}`
      );
    } catch (jwtError) {
      console.error(
        `Failed to verify asset token for musicId ${musicId}:`,
        jwtError
      );
    ...
    if (!public_id || !resource_type) {
      console.error(
        `Incomplete asset details in token for musicId: ${musicId}. Token payload:`,
        assetDetails
      );
      return res
        .status(500)
        .send("Internal server error: Incomplete asset information in token.");
    }
    ...
    const cloudinaryRequest = https.request(
      cloudinaryUrl,
      options,
      async (cloudinaryResponse) => {
        const { statusCode, headers: responseHeaders } = cloudinaryResponse;
        console.log(
          `Cloudinary response status: ${statusCode} for ${public_id}`
        );

        if (res.destroyed) {
          console.log(
            `Client connection closed before Cloudinary response for ${musicId}. Aborting.`
          );
        ...
        if (statusCode < 200 || statusCode >= 300) {
          console.error(
            `Error from Cloudinary for ${public_id}: Status ${statusCode}`
          );
          cloudinaryResponse.resume();
          if (res.headersSent) {
            console.warn(
              `Headers already sent for ${musicId}, but received error ${statusCode} from Cloudinary.`
            );
            return;
          }

          if (statusCode === 404)
            return res.status(404).send("Audio source file not found on CDN.");
          if (statusCode === 403)
            return res
              .status(403)
              .send("Forbidden accessing audio source on CDN.");
          if (statusCode === 416)
            return res.status(416).send("Range not satisfiable.");
          return res
            .status(502)
            .send(`Bad Gateway: Upstream server error (${statusCode})`);
        }
        if (res.headersSent) {
          console.warn(
            `Headers already sent for ${musicId}, but received success ${statusCode} from Cloudinary. Aborting pipe.`
          );
          cloudinaryResponse.resume();
          return;
        }
        ...
        const contentType =
          responseHeaders["content-type"] || `audio/${format || "mpeg"}`;
        res.setHeader("Content-Type", contentType);
        if (responseHeaders["content-length"])
          res.setHeader("Content-Length", responseHeaders["content-length"]);
        if (responseHeaders["accept-ranges"])
          res.setHeader("Accept-Ranges", responseHeaders["accept-ranges"]);
        if (responseHeaders["content-range"])
          res.setHeader("Content-Range", responseHeaders["content-range"]);

        res.writeHead(statusCode);
        console.log(
          `Piping stream for ${musicId} (${public_id}) to client... Status: ${statusCode}`
        );
        try {
          await pipeline(cloudinaryResponse, res);
          console.log(
            `Stream finished successfully for ${musicId} (${public_id}).`
          );
        } catch (pipeError) {
          console.error(
            `Stream piping error for ${musicId} (${public_id}):`,
            pipeError.code || pipeError.message
          );
    ...
    cloudinaryRequest.on("error", (error) => {
      console.error(
        `HTTPS request error to Cloudinary for ${public_id}:`,
        error
      );
    ...
    req.on("close", () => {
      ...
      if (!cloudinaryRequest.destroyed) {
        console.log(
          `Destroying Cloudinary request for ${musicId} due to client disconnect.`
        );
    ...
    console.error(
      `Unexpected error in stream controller for musicId ${musicId}:`,
      error
    );
    if (!res.headersSent && !res.destroyed) {
...
export const getMusicById = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res
      .status(400)
      .json({ message: "Invalid music identifier format." });
  }
  try {
...
export const updateMusic = async (req, res) => {
  ...
  try {
    ...
    if (music.is_deleted) {
      return res
        .status(404)
        .json({ message: "Cannot update a deleted music track." });
    }

    if (
      !music.collaborators ||
      music.collaborators.length === 0 ||
      !music.collaborators[0].user_id
    ) {
      console.warn(
        `Music track ${id} has missing primary collaborator user_id for authorization check.`
      );
      return res
        .status(500)
        .json({ message: "Cannot determine owner for update authorization." });
    }
    if (
      !music.collaborators[0].user_id.equals(
        req.user._id
      ) /* && !req.user.isAdmin */
    ) {
      console.warn(
        `User ${req.user._id} attempted to update music ${id} owned by ${music.collaborators[0].user_id}`
      );
      return res
        .status(403)
        .json({
          message:
            "Forbidden: You do not have permission to update this track.",
        });
    }
    ...
    if (Array.isArray(genre_id)) {
      updateData.genre_id = genre_id.filter((gid) =>
    ...
    if (Array.isArray(collaborators)) {
      ...
      collaborators.forEach((collab) => {
        if (collab && typeof collab.name === "string" && collab.name.trim()) {
          ...
          if (
            formattedCollab.user_id &&
            toString(formattedCollab.user_id) === toString(req.user._id)
          ) {
          ...
          console.warn(
            "Ignoring invalid collaborator object during update:",
            collab
          );
        }
      });
      if (!primaryFound) {
        console.warn(
          `Uploader ${req.user.username} (${req.user._id}) was not found in the submitted collaborator list during update. Re-adding as Primary Artist.`
        );
    ...
    console.log(
      `Updating music track ${id} by user ${req.user._id} with data:`,
      updateData
    );
    ...
    if (!updatedMusic) {
      return res
        .status(404)
        .json({ message: "Music track not found during update operation." });
    }
    ...
    if (error.name === "ValidationError") {
      return res
        .status(400)
        .json({
          message: "Validation Error updating music data.",
          errors: error.errors,
        });
    }
    if (error.name === "CastError") {
      return res
        ...
        .json({
          message:
            "Invalid data format provided for update (e.g., invalid ID).",
          field: error.path,
        });
    }
    return res
      .status(500)
      .json({ message: "Internal Server Error during music update." });
  }
};
...
export const deleteMusic = async (req, res) => {
  ...
  try {
    ...
    if (music.is_deleted) {
      ...
      return res
        .status(200)
        .json({ message: "Music track was already deleted." });
    }
    ...
    if (
      ...
      console.warn(
        `Music track ${id} has missing primary collaborator user_id for delete authorization check.`
      );
      return res
        .status(500)
        .json({ message: "Cannot determine owner for delete authorization." });
    }
    ...
    if (!isOwner && !isAdmin) {
      console.warn(
        `User ${req.user._id} (role: ${req.user.role}) attempted to delete music ${id} owned by ${music.collaborators[0].user_id}`
      );
      return res
        .status(403)
        .json({
          message:
            "Forbidden: You do not have permission to delete this track.",
        });
    }

    console.log(
      `Soft deleting music track ${id} by user ${req.user._id} (IsAdmin: ${isAdmin})`
    );
    ...
    if (updateResult.modifiedCount === 0 && updateResult.matchedCount === 1) {
      console.log(
        `Music track ${id} was likely already deleted before the update operation completed.`
      );
      return res
        .status(200)
        .json({ message: "Music track was already deleted." });
      ...
      console.error(
        `Failed to find music track ${id} during the delete (updateOne) operation.`
      );
      return res
        .status(404)
        .json({ message: "Music track not found during delete operation." });
    }
    ...
    return res
      .status(500)
      .json({ message: "Internal Server Error during music deletion." });
  }
};
...
export const searchMusic = async (req, res) => {
  ...
  console.log(
    `Music search requested by user: ${req.user?.username || "Unknown"}`
  );
  try {
    ...
    if (q && typeof q === "string" && q.trim()) {
      ...
      searchConditions.push({
        $or: [{ title: keywordRegex }, { "collaborators.name": keywordRegex }],
      });
    }

    if (artist && typeof artist === "string" && artist.trim()) {
      searchConditions.push({
        "collaborators.name": { $regex: artist.trim(), $options: "i" },
      });
    }
    ...
    console.log(
      "Executing music search with filter:",
    ...
    const formattedResults = musicList.map((music) => ({
      ...
      duration_ms: music.duration_ms,
      primary_artist_name: music.collaborators[0]?.name || "N/A",
      collaborators: music.collaborators.map((c) => ({
        name: c.name,
        role: c.role,
      })),
      like_count: music.like_count,
    ...
    if (error.name === "CastError" && error.path?.includes("_id")) {
      return res
        .status(400)
        .json({
          message: "Invalid ID format provided for genre or album filter.",
        });
    }
    return res
      .status(500)
      .json({ message: "Internal Server Error during music search." });
  }
};
...
export const listNewMusic = async (req, res) => {
  ...
  console.log(
    `Listing new music requested by user: ${req.user?.username || "Unknown"}`
  );
  try {
    const limitNum = parseInt(limit, 10);
    if (isNaN(limitNum) || limitNum <= 0 || limitNum > 100) {
      return res
        ...
        .json({
          message:
            "Invalid limit parameter. Must be a positive number (max 100).",
        });
    }
    ...
    return res
      .status(500)
      .json({ message: "Internal Server Error fetching new music." });
  }
};
